...
($-13.667$ for Lorenz, $-83.44$ for KS), and Chamfer/Hausdorff overlap
against the encoded attractor. The KS system uses Galerkin-truncated
evolution ($L=22$, 17 modes) with RK4 time integration at $dt=0.01$,
observed through a smooth observation function that lifts the 17 complex
(or 34 real) Fourier coefficients into a 64-D latent chart.
...
\subsection{Part 1b: validation on Kuramoto--Sivashinsky (34 DOF)}

To confirm the atlas findings generalize beyond Lorenz, we repeat the
five-arm comparison on the Kuramoto--Sivashinsky (KS) PDE, a canonical
spatiotemporally chaotic system with $L=22$, 17 Galerkin modes, true
divergence $-83.44$, and trace-free nonlinearity. The KS system is
maximally different from Lorenz: infinite-dimensional PDE (truncated to
34 real DOF), spatially extended, strong nonlinear mode coupling. The
same five predictor classes, the same training protocol, and the same
metrics are used.
...
\begin{tabular}{lcccc}
\toprule
arm & contraction & $|\mathrm{tr}/n - \mathrm{div}|$ & chamfer\_rel & $\|z\|_{\max}$ \\
\midrule
B unconstrained & $-21.7\pm1.4$ & 4043 & 13.5 & 6066 \\
C rigid Hamiltonian & $+0.03\pm0.01$ & 4065 & 0.31 & 9.7 \\
D naive metriplectic & $+0.03\pm0.16$ & 4065 & 0.57 & 16.7 \\
E fixed metriplectic & $\mathbf{-13.4\pm0.1}$ & 4052 & 0.90 & 14.0 \\
F + spectral align & NaN & NaN & $\infty$ & $\infty$ \\
\bottomrule
\end{tabular}
\caption{Kuramoto--Sivashinsky atlas (true divergence $-83.44$; 250
integration steps, 3 seeds). The same failure modes as Lorenz: the
rigid Hamiltonian and naive metriplectic arms sit on the dead saddle
(contraction $\approx 0$, error $\approx 4065$); the fixed metriplectic
arm captures meaningful dissipation (contraction $-13.4$, closest to
truth); the unconstrained MLP blows up (norm 6066). Spectral alignment
is numerically unstable on this stiffer system.}
\label{tab:ks}
\end{table}

\textbf{Reading.} The KS atlas confirms every Lorenz finding on a
fundamentally different system: (i)~rigid Hamiltonians freeze at
contraction $\approx 0$; (ii)~naive metriplectics sit on the dead saddle
(contraction $\approx 0$, $R$ never engages); (iii)~the fixed metriplectic
arm captures $\sim16\%$ of the true divergence (the harder system
explains the lower fraction vs.~Lorenz's $\sim101\%$); (iv)~overlap
metrics still reward frozen models (Chamfer $0.31$ for the dead
Hamiltonian). Two chaotic systems, same structural story: the failure
modes are properties of the predictor class, not the system.
...
\textbf{Two chaotic systems.} The Lorenz-63 atlas (5 arms, 3 seeds,
Table~\ref{tab:atlas}) and the Kuramoto--Sivashinsky atlas (5 arms,
3 seeds, 34 real DOF, Table~\ref{tab:ks}) show the same failure modes
on maximally different systems: rigid Hamiltonians freeze at
contraction $\approx 0$, naive metriplectics sit on the dead saddle
($R$ never engages), fixed metriplectics capture meaningful dissipation
($-13.4\pm0.1$ on KS, $-13.8\pm0.2$ on Lorenz vs their respective
true divergences). Generalization beyond a single system is the minimum
bar for a structural claim; we clear it.

...
end-to-end from a clean checkout: reconstruction loss falls, KL free bits
active, actor/critic losses finite, evaluation episodes complete, and the
stability audit runs --- the metriplectic arm's dissipation engages
($\mathrm{tr}(R)/n > 0$ and rising) inside the RL loop, mirroring the
Lorenz atlas. This validates the pipeline; it is not evidence about the
hypothesis. The headline comparison (Table~\ref{tab:crafter} and the
stability curves) is produced by the shipped T4 runs.
...
\begin{tabular}{lccc}
\toprule
arm & return @ 20k & drift$/$\|z\|$^2$ & raw KL \\
\midrule
RSSM (DreamerV3) & \textit{--} & \textit{--} & \textit{--} \\
Metriplectic (E-arm) & \textit{--} & \textit{--} & \textit{--} \\
\bottomrule
\end{tabular}
\caption{Crafter sample efficiency and latent stability (3 seeds,
mean $\pm$ std), to be filled from the T4 runs (see
\texttt{kaggle/README.md}). Placeholders are deliberate: we do not ship
numbers we have not run.}
\label{tab:crafter}
\end{table}
